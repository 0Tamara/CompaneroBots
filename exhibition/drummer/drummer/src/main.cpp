...
void loadDatabase()  //read the file and save song starting lines (empty lines)
{
...
void loadSongHeader(int which)
{
  ...
  while (database.available())  //read the line
  {
    ...
    {
      case 0:
        current_song.song_number = which;
        break;
      case 1:
...
void OnDataRecieve(const uint8_t * mac, const uint8_t *incoming_data, int len)
{
  if(!busy_playing)
  {
    memcpy(&recieved_mes, incoming_data, sizeof(recieved_mes));
    Serial.printf("Recieved song: %d; bar: %d\n", recieved_mes.song, recieved_mes.bar);
    if(recieved_mes.song > 0)
    {
      if(recieved_mes.song != current_song.song_number)  //if the song is new
      {
        loadSongHeader(recieved_mes.song);
        current_bar.end = 0;
      }
      if(recieved_mes.bar == current_bar.end)  //if the bar type is new
        loadBar();
      playBar();
    }
  }
