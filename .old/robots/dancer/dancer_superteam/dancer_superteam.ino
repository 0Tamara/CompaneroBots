...
void OnDataRecv(const uint8_t* mac, const uint8_t* incomingData, int len) {
  ...
  if (recv_data.value == 1) {
    ...
    to_left_side(255);
    delay(5000); //oblbovak prva cast
    to_right_side(255);
    delay(15000); //jejo ono ten trik vysiel
    forward(255);
    delay(30000);
    right(255); //po gole vsetci jasaju 
    right(255); // otacame sa
    delay(2000);
    while (1)
    {   
        start = millis();
        servoRamp(130, leftShoulder);
        servoRamp(180, leftShoulder);
